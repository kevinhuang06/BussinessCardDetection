...
def findAndTransform(processed, original, scale):
    ...
    cv2.rectangle(original, (uLeft[1],uLeft[0]),(lRight[1],lRight[0]), 3, 2,0)
    return angle_check, dst,original
...
def findCard(img):
    ...
    good, cropped,boxed = findAndTransform(edits, img, scale)
    return good, cropped,boxed

def main():
    card_dir = "D:\\sohu\\Colleague\\chenghao\\bizcard\\"
    #card_dir= 'D:\\BaiduYunDownload\\bizcard'
    #box_card_dir = "D:\\sohu\\Colleague\\chenghao\\box_stanford\\"
    crop_card_dir = 'D:\\sohu\\Colleague\\chenghao\\crop_stanford\\'
    crop_card_dir = 'D:\\sohu\\Colleague\\chenghao\\crop_bizcard\\'
    
    imgs = os.listdir(card_dir)

    for img in imgs:
        image_o = cv2.imread(os.path.join(card_dir,img))
        print os.path.join(card_dir,img)
        if 'baiduyun' in os.path.join(card_dir,img):
            continue
        #Shows image before card extration run
        #plt.imshow(cv2.cvtColor(image_o, cv2.COLOR_BGR2RGB))
        #plt.show()
        try:
            good, dst,boxed = findCard(image_o)
        except Exception,ex:
            print ex
            continue
        #print good,os.path.join(box_card_dir,img)
        #plt.imshow(cv2.cvtColor(boxed, cv2.COLOR_BGR2RGB))
        #plt.show()
        cv2.imwrite(os.path.join(crop_card_dir,img), dst)
        #cv2.imwrite(os.path.join(box_card_dir,img), boxed)
        if good and None:
            plt.subplot(121)
